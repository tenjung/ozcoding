...
const keywords = ["포스터", "로고", "리플렛", "포트폴리오", "상세페이지", "노트폴리오"];
const menu = [
  {
    label: "발견",
    newest: false,
...
function AppHeader({ onSetSearchValue, onFetchApi, onSetCategory }) {
  const handleSearch = () => {
    onFetchApi();
  };
        ...
        <div className="w-full h-full flex items-center justify-between">
          <div className="w-full flex items-center gap-4">
            <Sheet>
              ...
              <SheetContent side="left" className="flex flex-col px-8 pb-8 gap-8 overflow-y-scroll">
                ...
                <div className="flex flex-col gap-6">
                  {menu.map((item, index) => {
                    return (
                      <div key={index} className={`h-full flex items-center gap-1 font-medium`}>
                ...
                <div className="flex flex-col gap-6">
                  ...
                  <Separator />
                  <div className="flex flex-col gap-2">
                    <p className="text-xs font-semibold mb-3">(주)스터닝 사업자 정보</p>
                    <p className="text-xs text-neutral-500">사업자 등록번호 : 120-87-69298</p>
                    <p className="text-xs text-neutral-500">직업정보제공: J1200020190003</p>
                    <p className="text-xs text-neutral-500">대표자명 : 김승환</p>
                    <p className="text-xs text-neutral-500">전화번호:070-8733-5858</p>
                    <p className="text-xs text-neutral-500">주소 : 서울특별시 강남구 봉은사로112길 6 2F</p>
                    <p className="text-xs font-semibold mt-3">ⓒ 2020 STUNNING INC.</p>
                  </div>
                </div>
              </SheetContent>
            </Sheet>
          ...
          <div className="flex items-center gap-2">
            ...
            <Drawer>
              ...
              <DrawerContent className="!max-h-[100vh] h-screen mt-0">
                <DrawerHeader>
                  <div className="flex items-center w-full">
                    {/* 검색창 자체 */}
                    <div className="flex items-center border rounded-full px-3 bg-neutral-50 flex-1">
                      <Search size={18} className="text-neutral-400 mr-2" />
                      <Input
                        placeholder="230,000개 이상의 크리에이티브 검색"
                        onChange={(event) => onSetSearchValue(event.target.value)}
                        className="flex-1 bg-transparent placeholder:text-neutral-400 outline-0 border-none focus-visible:ring-0 py-6"
                      />
                    </div>
                    <DrawerClose>
                      <Button variant="outline" className=" rounded-full border-none text-2xl">
                        <X />
                      </Button>
                    </DrawerClose>
                  </div>
                ...
                <div className="flex flex-col gap-2 p-4">
                  <p className="text-xs font-semibold text-gray-500">노트폴리오 추천 검색어</p>

                  <div className="grid grid-cols-5 gap-3">
                    {keywords.map((keyword, i) => (
                      <Button key={i} variant="outline" className="rounded-full font-bold text-xs">
                        {keyword}
                      </Button>
                    ))}
                  </div>
                ...
                <Separator />
                <div className="flex flex-col gap-2 p-4">
                  <p className="text-xs font-semibold text-gray-500">노트폴리오 추천 콘텐츠</p>
                </div>
        ...
        <div className="flex items-center gap-2">
          <div className="flex items-center border px-3 rounded-full bg-neutral-50">
            ...
            <Input
              ...
              onKeyDown={(event) => {
                if (event.key === "Enter") {
                  onSetCategory(event.target.value); // 사용자가 입력한 값 전달
                }
              }}
